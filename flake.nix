        ...
        generateApps = flake:
          let
            ...
            mkDeployScript = { machine, dryRun }: let
              ...
              host = n.host;
              # if using a build host, 
              buildHost = n.buildHost or host;
              ...
              '')
                ...
                ( set -x; NIX_SSHOPTS="-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -T -A" ${flock} -w 60 /dev/shm/nixinate-${machine} ${nixos-rebuild} ${nixOptions} ${switch} --flake ${flake}#${machine} ${optionalString (host!=buildHost) "--build-host ${user}@${buildHost}"} --target-host ${user}@${host} --use-remote-sudo ${optionalString substituteOnTarget "-s"} )
